...
- **Python 実行環境**（例：Anaconda）  
  LichtFeld Studio を自分でビルドする場合や 360-to-RealityScan を Python スクリプトとして直接実行する場合に必要です。  
  LichtFeld Studio・360-to-RealityScan ともにビルド済み実行形式（.exe）を使用する場合はインストール不要です。
