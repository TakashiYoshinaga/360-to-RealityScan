...
- **Python 実行環境**（例：Anaconda）  
  ...
  360-to-RealityScan のビルド済み実行形式（.exe）を使用する場合はインストール不要です。[こちら](https://github.com/TakashiYoshinaga/360-to-RealityScan/releases)から **SphereSfMToRS.exe** をダウンロードしてください。
...
> **ℹ️ この手順はオプションです**  
> AutoMasker を使用しない場合は、[2.3 SfM の設定](#23-sfm-の設定) へスキップしてください。
...
1.　[Releases](https://github.com/TakashiYoshinaga/360-to-RealityScan/releases) から **`SphereSfMToRS.exe`** をダウンロードし、ダブルクリックで起動します（Anaconda・Python 不要）

   初回起動時はウィンドウ下部の **FFmpeg (.exe) path** 欄に `ffmpeg.exe` のパスを入力し 💾 で保存します

> [!NOTE]
> **Python スクリプトとして実行する場合**  
> ターミナル（Python 実行環境）を開き、以下を実行します。
> ```bash
> cd C:\path\to\360-to-RealityScan
> python spheresfm_to_realityscan.py
> ```
> スクリプトの取得・環境設定は https://github.com/TakashiYoshinaga/360-to-RealityScan の README を参照してください。
...
![SphereSfM Converter の変換完了画面](https://raw.githubusercontent.com/TakashiYoshinaga/360-to-RealityScan/main/Documents/Images/014-SphereSfMConverter_3.jpg)
