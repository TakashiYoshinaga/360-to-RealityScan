...
[前回の記事（基本手順編）](../Basic/article.md)では、筆者が趣味でのGaussian Splattingで運用しているワークフローの最小構成をご紹介しました。今回は、その手順にほんの少しの工夫を加えるだけで、クオリティをさらに引き上げる方法を解説します。
...
> **💡 ヒント**  
> 手ぶれ補正を ON にしておくと、カメラが傾いても自動で垂直補正がかかります。今回のワークフローでは垂直保持が前提となっているため、手ぶれ補正 ON がおすすめです（前後カメラのスティッチ領域にわずかな歪みが生じる場合があります）。
...
5. **Exporter Setting** ダイアログで以下を設定します
...
続いて、**カメラの位置姿勢情報**を COLMAP 形式で出力します。
...
5. **Export transformation settings → Scene transformation** を開きます
