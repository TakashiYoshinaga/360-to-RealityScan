...
# ─────────────────────────────────────────────
# Config (saved next to the exe / script)
# ─────────────────────────────────────────────
def _get_config_path() -> Path:
    """Returns the path to config.json, placed next to the exe or script."""
...
def check_ffmpeg() -> bool:
    """Check whether FFmpeg is available at FFMPEG_EXE."""
    try:
...
class MetashapeToRSApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.ffmpeg_var: tk.StringVar = None  # type: ignore
    ...
    def _build_ui(self):
        ...
        row += 1
        ttk.Label(files_frame, text="FFmpeg (.exe) path:").grid(
            row=row, column=0, sticky=tk.W, pady=3
        )
        self.ffmpeg_var = tk.StringVar(value=FFMPEG_EXE)
        ttk.Entry(files_frame, textvariable=self.ffmpeg_var, width=45).grid(
            row=row, column=1, sticky=tk.EW, padx=5
        )
        ffmpeg_btn_frame = ttk.Frame(files_frame)
        ffmpeg_btn_frame.grid(row=row, column=2, sticky=tk.E)
        ttk.Button(ffmpeg_btn_frame, text="📄", width=3,
                   command=self._browse_ffmpeg).pack(side=tk.LEFT)
        ttk.Button(ffmpeg_btn_frame, text="💾", width=3,
                   command=self._save_ffmpeg_path).pack(side=tk.LEFT)
    ...
    def _save_ffmpeg_path(self):
        global FFMPEG_EXE
        path = self.ffmpeg_var.get().strip()
        FFMPEG_EXE = path if path else "ffmpeg"
        self.ffmpeg_var.set(FFMPEG_EXE)
        config = _load_config()
        config["ffmpeg_path"] = FFMPEG_EXE
        try:
            _save_config(config)
            messagebox.showinfo("Saved", f"FFmpeg path saved:\n{FFMPEG_EXE}")
        except Exception as e:
    ...
    def _start_conversion(self):
        ...
        # FFmpeg validation (skip if crop is disabled)
        if not self.skip_crop_var.get() and not check_ffmpeg():
            messagebox.showerror(
                "FFmpeg not found",
                f"FFmpeg executable not found:\n{FFMPEG_EXE}\n\n"
                "Please set the correct path in 'FFmpeg (.exe) path' and click 💾 to save.",
            )
            return
