...
提供されている PowerShell スクリプトを使って、ソースから Windows 実行ファイルをビルドできます。

### 前提条件

- [Anaconda](https://www.anaconda.com/download) または [Miniconda](https://docs.conda.io/en/latest/miniconda.html)
- **Python 3.10 以上** と **NumPy** がインストールされた conda 環境（上記の [セットアップ](#セットアップ) を参照）
- **ffmpeg は exe に同梱されません**（ライセンスの都合）。使用時は GUI 上でパスを手動設定します。  
  Windows 用 ffmpeg のダウンロード: https://github.com/BtbN/FFmpeg-Builds/releases
...
conda 環境名が `gs_env` でない場合は `-CondaEnv` で指定します。

```powershell
.\Build\build.ps1 -CondaEnv myenv
```

一方のツールだけをビルドすることも可能です。

```powershell
.\Build\build.ps1 -Target metashape   # MetashapeToRS.exe のみ
.\Build\build.ps1 -Target spheresfm   # SphereSfMToRS.exe のみ
```

出力ファイルは `Build\dist\` に生成されます。
...
> conda は PATH または一般的なインストール場所から自動検出されます。  
> PyInstaller が未インストールの場合は自動的にインストールされます。